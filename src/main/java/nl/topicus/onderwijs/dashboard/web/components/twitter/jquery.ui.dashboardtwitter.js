(function($, undefined) {
	...
	$.widget("ui.dashboardTwitter", {
		...
		_initialRead : function() {
			...
			this._initTwitter(this.options.timelineUrl,
					this._addItemsToTimeline, this._animateTimeline);
			this._initTwitter(this.options.mentionsUrl,
					this._addItemsToMentions, this._animateMentions);
		},
		...
		_animateTimeline : function() {
			this._animate("div.timeline .list", this.timelineGeneration,
					this.options.timelineUrl, this._addItemsToTimeline,
					this._animateTimeline, 20)
		},

		_animateMentions : function() {
			this._animate("div.mentions .list", this.mentionsGeneration,
					this.options.mentionsUrl, this._addItemsToMentions,
					this._animateMentions, 15)
		},

		_animate : function(selector, generation, url, addFunction,
				animateFunction, speed) {
			var self = this;
			...
			if (width == 0) {
				this.element.oneTime("10000ms", "start-marque", function() {
					self._initTwitter.apply(self, [ url, addFunction,
							animateFunction ]);
				});
				return;
			}

			var duration = (width * speed) + "ms";
			list.css({
			...
			this.element.oneTime(((width - 1400) * speed) + "ms",
					"update-marque", function() {
						$.getJSON(url, response);
					});
			this.element.oneTime((width * speed) + "ms", "rescroll-marque",
					function() {
						animateFunction.apply(self, []);
					});
		},
		...
		_addItemsToTimeline : function(data) {
			this._addNewItems("div.timeline .list", this.timelineGeneration,
					data);
			this.timelineGeneration++;
		},

		_addItemsToMentions : function(data) {
			this._addNewItems("div.mentions .list", this.mentionsGeneration,
					data);
			...
			if (data && data.length > 0) {
				...
				while (list.width() < 1500 + listStart) {
					$.each(data, function(index, value) {
						...
						var time = $("<span class='time'>" + value.timeAsString
								+ "</span>");
						var user = $("<span class='user'>" + value.user
								+ "</span>");
						var text = $("<span class='text'>" + value.text
								+ "</span>");
